...
import { useState, Suspense, useMemo, useRef, DragEvent, ChangeEvent, useCallback } from "react"
...
function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Save JWT so the user is logged in after registration
      extractAndSaveSession(response)
      setIsSuccess(true)
    ...
    if (role === "candidate") {
      ...
      return (
        <div className="w-full flex flex-col items-center justify-center gap-4 py-8 text-center animate-in fade-in zoom-in-95">
          ...
          <Button onClick={() => router.push("/recruiter")} className="w-full h-11 gradient-violet border-none text-white font-bold hover:opacity-90">
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      <Button
        variant="outline"
        className="w-full h-11 rounded-lg gap-2.5 font-medium border-border/60 hover:bg-muted/50"
        type="button"
